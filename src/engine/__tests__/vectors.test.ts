...
const VECTORES: Vector[] = [
...

  {
    ...
    cronograma: {
      ...
      semanas: [40, 52],
      diet_breaks: 4,
      fecha_min: '2027-06-14',
      fecha_max: '2027-09-06',
      precision_fecha: 'mes',
      tramo_12sem: [5.0, 7.0],
    },
...

  {
    n: '4',
    titulo:
      'Mujer 52 años, obesidad grado I, estimación visual, objetivo "no lo sé", diabetes, 5 comidas',
    inputs: {
      ...BASE,
      sexo: 'mujer',
      edad: 52,
    ...
    cronograma: {
      ...
      semanas: [37, 46],
      diet_breaks: 4,
      fecha_min: '2027-05-24',
      fecha_max: '2027-07-26',
      precision_fecha: 'mes',
      tramo_12sem: [3.5, 5.0],
    },
...

  {
    ...
    cronograma: {
      ...
      semanas: [17, 20],
      diet_breaks: 0,
      fecha_min: '2027-01-04',
      fecha_max: '2027-01-25',
      precision_fecha: 'mes',
      tramo_12sem: [2.5, 3.0],
    },
...

  {
    n: '7',
    titulo:
      'Hombre 40 años, trabajo físico, low-carb, endomorfo, clima caluroso, peso objetivo 80 kg',
    inputs: {
      ...BASE,
      sexo: 'hombre',
      edad: 40,
    ...
    cronograma: {
      ...
      semanas: [20, 23],
      diet_breaks: 2,
      fecha_min: '2027-01-25',
      fecha_max: '2027-02-15',
      precision_fecha: 'mes',
      tramo_12sem: [7.0, 8.0],
    },
...

  {
    n: '8',
    titulo:
      'Mujer 29 años, sedentaria, ritmo agresivo, vegana, peso objetivo irreal (48 kg), 2 comidas',
    inputs: {
      ...BASE,
      sexo: 'mujer',
      edad: 29,
...

  {
    ...
    cronograma: {
      ...
      semanas: [58, 84],
      diet_breaks: 6,
      fecha_min: '2027-10-18',
      fecha_max: '2028-04-17',
      precision_fecha: 'mes',
      tramo_12sem: [3.0, 4.0],
    },
...

  {
    ...
    cronograma: {
      ...
      semanas: [30, 37],
      diet_breaks: 3,
      fecha_min: '2027-04-05',
      fecha_max: '2027-05-24',
      precision_fecha: 'mes',
      tramo_12sem: [5.5, 7.0],
    },
...
describe('Caso 0 — exclusiones del paso 0', () => {
  ...
  it('embarazo o lactancia → EXCL_EMBARAZO_LACTANCIA', () => {
    expect(calcular(con({ sexo: 'mujer', peso_kg: 60, embarazo_lactancia: true })).excluido).toBe(
  ...
  it("'tca' con IMC 17,9 → EXCL_TCA_RIESGO", () => {
    expect(calcular(con({ altura_cm: 180, peso_kg: 58, condiciones: ['tca'] })).excluido).toBe(
      'EXCL_TCA_RIESGO',
    )
  })
  it('el cribado positivo o evitado añade tca y puede excluir', () => {
    expect(calcular(con({ altura_cm: 180, peso_kg: 58, cribado_tca: 'positivo' })).excluido).toBe(
      'EXCL_TCA_RIESGO',
    )
    expect(calcular(con({ altura_cm: 180, peso_kg: 58, cribado_tca: 'evitado' })).excluido).toBe(
      'EXCL_TCA_RIESGO',
    )
    expect(
      calcular(con({ altura_cm: 180, peso_kg: 58, cribado_tca: 'negativo' })).excluido,
    ).toBeUndefined()
  })
...
describe('§1 — validación de rangos y dominios', () => {
  ...
  it('n_comidas fuera del conjunto', () => {
    expect(calcular(con({ n_comidas: 7 as unknown as Inputs['n_comidas'] })).errores).toContain(
      'n_comidas',
    )
  })
  ...
  it('medidas sin cadera en mujer', () => {
    const r = calcular(
      con({
        sexo: 'mujer',
        peso_kg: 60,
        grasa: { metodo: 'medidas', cuello_cm: 32, cintura_cm: 72 },
      }),
    )
  ...
  it("con tipo 'ninguno' no se validan minutos ni intensidad", () => {
    const r = calcular(
      con({ entrenamiento: ent({ tipo: 'ninguno', minutos_sesion: 0, momento: null }) }),
    )
    expect(r.excluido).toBeUndefined()
  })
...
describe('Casos borde', () => {
  ...
  it('momento null deja todas las comidas sin peri y sin desplazar hidratos', () => {
    ...
    const sinMomento = calcular(
      con({
        entrenamiento: ent({ tipo: 'fuerza', dias_semana: 4, minutos_sesion: 60, momento: null }),
        n_comidas: 4,
      }),
    )
  ...
  it('low_carb con kcal muy bajas respeta el mínimo de 75 g de hidratos y el techo de grasa', () => {
    ...
    expect(9 * r.macros.grasa_g).toBeLessThanOrEqual(0.5 * r.kcal)
  ...
  it('edad 65 exacta baja el techo de déficit al 20 % y sube el suelo de IMC del peso objetivo', () => {
    ...
    expect(r65.tdee.valor - r65.kcal).toBeLessThanOrEqual(0.2 * r65.tdee.valor + 10)
  ...
  it('bandas de grasa en sus bordes exactos', () => {
    const banda = (valor: number, sexo: Inputs['sexo']) =>
      calcular(
        con({
          sexo,
          peso_kg: sexo === 'hombre' ? 70 : 60,
          grasa: { metodo: 'conocido', valor, fuente: 'estimado' },
        }),
      ).grasa.banda
  ...
  it('medidas Navy al borde: x < 15 en hombre cae a CUN-BAE con aviso', () => {
    const r = calcular(
      con({
        altura_cm: 180,
        peso_kg: 85,
        grasa: { metodo: 'medidas', cuello_cm: 40, cintura_cm: 54.9 },
      }),
    )
  ...
  it('medidas Navy al borde: resultado fuera de 3–60 se descarta', () => {
    const r = calcular(
      con({
        altura_cm: 130,
        peso_kg: 100,
        grasa: { metodo: 'medidas', cuello_cm: 25, cintura_cm: 200 },
      }),
    )
    expect(r.avisos).toContain('WARN_MEDIDAS_INVALIDAS')
    expect(r.grasa.metodo_efectivo).toBe('desconocido')
  ...
  it('medidas Navy válidas con mucha discrepancia avisan pero se usan', () => {
    const r = calcular(
      con({
        ...
        altura_cm: 175,
        peso_kg: 110,
        grasa: { metodo: 'medidas', cuello_cm: 35, cintura_cm: 90 },
      }),
    )
...
describe('Invariantes de seguridad sobre los diecinueve vectores', () => {
      ...
      for (const [disparador, suprimidos] of pares) {
        if (r.avisos.includes(disparador)) {
          for (const s of suprimidos)
            expect(r.avisos, `caso ${v.n}: ${disparador} vs ${s}`).not.toContain(s)
        }
      }
    }
  })
  ...
  it("con 'tca' no se emite ningún aviso de la lista protegida", () => {
    ...
    const r = calcular(
      con({ altura_cm: 165, peso_kg: 78, sexo: 'mujer', condiciones: ['tca'], objetivo: 'perder' }),
    )
